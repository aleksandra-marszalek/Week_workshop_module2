	...
	public static void userManager () {
		...
		// show the options
			System.out.println("Please choose right command:\n"
					...
					+ "assign - to assign user to a user group,\n"
					+ "quit - to quit the user management system.");
			Scanner scan = new Scanner(System.in);
			String input1 = scan.nextLine();
			while (!input1.equalsIgnoreCase("add") && !input1.equalsIgnoreCase("edit") && 
				   !input1.equalsIgnoreCase("delete") && !input1.equalsIgnoreCase("quit") && !input1.equalsIgnoreCase("assign")) {
				System.out.println("I don't understand. Please insert the right command: add/edit/delete/assign/quit");
		...
		//ADD			
			...
			// EDIT
				...
				//ASSIGN		
					...
					} else if (input1.equalsIgnoreCase("assign")) {
						System.out.println("Insert the way you would like to search for the user:\n"
								+ "Email - if you want to find the user by email,\n"
								+ "Id - if you want to find the user by id,\n"
								+ "Name - if you want to find the user by name and surname.");
						...
						int userIdtoEdit;
						if (input2.equalsIgnoreCase("email")) {
							System.out.println("Insert the email of the user you want to assign to a group: ");
							String email = scan.nextLine();
							userIdtoEdit = UserDao.readByEmail(email).getId();
						} else if (input2.equalsIgnoreCase("name")) {
							System.out.println("Insert the name of the user you want to assign to a group (in format: name surname): ");
							String name = scan.nextLine();
							userIdtoEdit = UserDao.readByEmail(name).getId();
						} else {
							System.out.println("Please insert the id of the user You want to assign to a group: ");
							userIdtoEdit = Integer.parseInt(scan.nextLine());
						}
						User userNew = UserDao.readById(userIdtoEdit);	
						if (userNew.getId()==0) {
			...
			// ask if there is anything else the user would like to do - if quit - the program stops
				System.out.println("Would you like to do anything else? Please choose right command: add/edit/delete/assign/quit: ");
				input1 = scan.nextLine();
